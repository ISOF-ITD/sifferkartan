...
def main():
    if len(sys.argv) < 4:
        ...
        print(
            "Usage processing original: python program input-folder-original output-folder json-input-ekonomiska-kartan json-input-info.json"
        )
        sys.exit(1)
    if len(sys.argv) > 4:
    ...
    INPUT_FOLDER, OUTPUT_FOLDER, JSON_GEOREFERENCE = (
        sys.argv[1],
        sys.argv[2],
        sys.argv[3],
    )
...
def batch_process_maps(
    input_folder: str, output_folder: str, json_path: str, log_level: int = 0
):
    print(input_folder)
...
def process_original(tif_path, geo_info, map_info, kartbladsid, output_path):
    ...
    if len(polygon_corners) != 4:
        raise ValueError(
            f"Expected 4 corners, found {len(polygon_corners)}. "
...
def process_tif(tif_path, geo_info, kartbladsid, output_path):
    ...
    if coords is None:
        raise ValueError(
            f"kartbladsid '{kartbladsid}' not found in geo_info ('{just_kartbladsid}')"
        )
